...
public class MarkovModel {
    ...
    public String getRandomText(int numChars){//his method generates and returns random text that is numChars long.
        ...
        int index = myRandom.nextInt(myText.length()-N);// -1 want .length houdt geenr ekening met positie 0, je hebt nog steeds de follow up letters nodig om de string te kunnen capteren
        String key = myText.substring(index, index+N);
        sb.append(key);
        
        for(int k=0; k < numChars-N; k++){// -1 want anders wordt de output langer dan numchars aangezien al een letter is teogevoegd aan de stringbuilder
