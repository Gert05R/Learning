...
public class Movie {
    ...
    String aCountry, String aPoster, int theMinutes) {
        // just in case data file contains extra whitespace
        id = anID.trim();//trim trims whitespaces in the and or beginning of the string
        title = aTitle.trim();
        year = Integer.parseInt(aYear.trim());
        genres = theGenres;
        director = aDirector;
    ...
    // Returns a string of the item's information A toString method for representing movie information as a String so it can easily be printed.
